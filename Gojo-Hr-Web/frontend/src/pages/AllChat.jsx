...
const ROLE_META = {
  ...
  management: { label: 'Management', node: 'Management' },
}
...
const normalizeTab = (value) => {
  ...
  if (tab === 'management' || tab === 'admin' || tab === 'admins') return 'management'
  return 'teacher'
}
...
export default function AllChat() {
  ...
  const [baseNodes, setBaseNodes] = useState({ teacher: {}, finance: {}, management: {} })
  const [contactsByTab, setContactsByTab] = useState({ teacher: [], finance: [], management: [] })
  ...
  useEffect(() => {
    ...
    const loadBaseData = async () => {
      ...
      try {
        const [contextRes, usersRes, teachersRes, financeRes, managementRes] = await Promise.all([
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(chatsRef, (snapshot) => {
      ...
      setContactsByTab({
        ...
        management: buildContacts('management'),
      })
    })
        ...
        >
          <style>{`
            ...
            .hr-chat-hero {
              ...
              background:
                radial-gradient(circle at top right, rgba(96, 165, 250, 0.20), transparent 34%),
                linear-gradient(135deg, #f8fbff 0%, #e9f2ff 52%, #f8fbff 100%);
            ...
            .hr-chat-thread-head {
              ...
              background: linear-gradient(180deg, rgba(255,255,255,0.96) 0%, rgba(243,247,255,0.96) 100%);
            ...
            .hr-chat-body {
              ...
              background:
                radial-gradient(circle at 20% 20%, rgba(191, 219, 254, 0.22), transparent 18%),
                radial-gradient(circle at 80% 0%, rgba(96, 165, 250, 0.12), transparent 20%),
                linear-gradient(180deg, #edf4ff 0%, #f8fbff 45%, #eef6ff 100%);
            }
            ...
            .hr-chat-bubble.mine::after {
              right: -5px;
              background: linear-gradient(135deg, #1473f6 0%, #0d65e8 100%);
            ...
            .hr-chat-composer {
              ...
              border: 1px solid rgba(209, 223, 248, 0.95);
            ...
            .hr-chat-send {
              ...
              border: none;
              border-radius: 50%;
              background: linear-gradient(135deg, #2a7fff, #0d65e8);
              color: #fff;
              cursor: pointer;
              display: inline-flex;
              align-items: center;
            ...
            <section className="hr-chat-hero">
              <div className="hr-chat-topbar">
                ...
                <div className="hr-chat-hero-copy">
                  <h1>All Chat</h1>
                  <p>
                    HR can message staff teams directly from one workspace. This page is limited to employee communication with Teachers, Finance, and Management, and uses the shared realtime chat node so it stays compatible with the other panels.
                  </p>
                </div>
              </div>
              ...
              <section className={`hr-chat-thread ${isMobile && !selectedChatUser ? 'mobile-hidden' : ''}`}>
                {selectedChatUser ? (
                  ...
                  <div className="hr-chat-empty-thread">
                    ...
                    <p>Pick a teacher, finance officer, or management contact from the list.</p>
                  </div>
                )}
              </section>
            </section>
